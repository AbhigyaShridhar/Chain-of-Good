  ...
  init: async function() {
    ...
    $.getJSON('../students.json', function(data) {
      ...
      for (i = 0; i < data.length; i ++) {
        ...
        studentTemplate.find('.student-domain').text(data[i].domain);
        studentTemplate.find('.student-age').text(data[i].age);
        studentTemplate.find('.student-location').text(data[i].location);
        studentTemplate.find('.btn-adopt').attr('data-id', data[i].id);
